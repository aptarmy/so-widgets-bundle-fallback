...
abstract class SiteOrigin_Widget_Base_Slider extends SiteOrigin_Widget {
	...
	function initialize() {
		...
		$frontend_scripts[] = array(
			...
			plugin_dir_url( SOW_FALLBACK_BUNDLE_BASE_FILE ) . 'js/jquery.cycle' . SOW_BUNDLE_JS_SUFFIX . '.js',
			array( 'jquery' ),
			SOW_FALLBACK_BUNDLE_VERSION
		);
		if( function_exists('wp_is_mobile') && wp_is_mobile() ) {
			$frontend_scripts[] = array(
				'sow-slider-slider-cycle2-swipe',
				plugin_dir_url( SOW_FALLBACK_BUNDLE_BASE_FILE ) . 'js/jquery.cycle.swipe' . SOW_BUNDLE_JS_SUFFIX . '.js',
				array( 'jquery' ),
				SOW_FALLBACK_BUNDLE_VERSION
			);
		}
		$frontend_scripts[] = array(
			'sow-slider-slider',
			plugin_dir_url( SOW_FALLBACK_BUNDLE_BASE_FILE ) . 'js/slider/jquery.slider' . SOW_BUNDLE_JS_SUFFIX . '.js',
			array( 'jquery' ),
			SOW_FALLBACK_BUNDLE_VERSION
		);
		...
		$this->register_frontend_styles(
			array(
				array(
					'sow-slider-slider',
					plugin_dir_url( SOW_FALLBACK_BUNDLE_BASE_FILE ) . 'css/slider/slider.css',
					array(),
					SOW_FALLBACK_BUNDLE_VERSION
				)
			)
	...
	function control_form_fields(){
		return array(
			'speed' => array(
				'type' => 'number',
				'label' => __('Animation speed', 'seed'),
				'description' => __('Animation speed in milliseconds.', 'seed'),
				'default' => 800,
			),

			'timeout' => array(
				'type' => 'number',
				'label' => __('Timeout', 'seed'),
				'description' => __('How long each frame is displayed for in milliseconds.', 'seed'),
				'default' => 8000,
			),

			'nav_color_hex' => array(
				'type' => 'color',
				'label' => __('Navigation color', 'seed'),
				'default' => '#FFFFFF',
			),

			'nav_style' => array(
				'type' => 'select',
				'label' => __('Navigation style', 'seed'),
				'default' => 'thin',
				'options' => array(
					'ultra-thin' => __('Ultra thin', 'seed'),
					'thin' => __('Thin', 'seed'),
					'medium' => __('Medium', 'seed'),
					'thick' => __('Thick', 'seed'),
					'ultra-thin-rounded' => __('Rounded ultra thin', 'seed'),
					'thin-rounded' => __('Rounded thin', 'seed'),
					'medium-rounded' => __('Rounded medium', 'seed'),
					'thick-rounded' => __('Rounded thick', 'seed'),
				)
			),

			'nav_size' => array(
				'type' => 'number',
				'label' => __('Navigation size', 'seed'),
				'default' => '25',
			),

			'swipe' => array(
				'type' => 'checkbox',
				'label' => __( 'Swipe Control', 'seed' ),
				'description' => __( 'Allow users to swipe through frames on mobile devices.', 'seed' ),
				'default' => true,
			)
		);
	}
	...
	function video_form_fields(){
		return array(
			'file' => array(
				'type' => 'media',
				'library' => 'video',
				'label' => __('Video file', 'seed'),
			),

			'url' => array(
				'type' => 'text',
				'sanitize' => 'url',
				'label' => __('Video URL', 'seed'),
				'optional' => 'true',
				'description' => __('An external URL of the video. Overrides video file.', 'seed')
			),

			'format' => array(
				'type' => 'select',
				'label' => __('Video format', 'seed'),
				'options' => array(
					'video/mp4' => 'MP4',
					'video/webm' => 'WebM',
					'video/ogg' => 'Ogg',
