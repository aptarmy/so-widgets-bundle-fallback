...
function siteorigin_widget_post_selector_enqueue_admin_scripts() {
	if ( ! wp_script_is( 'siteorigin-widget-admin-posts-selector' ) ) {
		...
		wp_enqueue_style( 'siteorigin-widget-admin-posts-selector', theme_dir_url(SOW_FALLBACK_BUNDLE_BASE_FILE).'base/css/post-selector.css', array(), SOW_FALLBACK_BUNDLE_VERSION );
		wp_enqueue_script( 'siteorigin-widget-admin-posts-selector', theme_dir_url(SOW_FALLBACK_BUNDLE_BASE_FILE).'base/js/posts-selector' . SOW_BUNDLE_JS_SUFFIX . '.js', array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-autocomplete', 'underscore', 'backbone' ), SOW_FALLBACK_BUNDLE_VERSION, true );
		...
		wp_localize_script( 'siteorigin-widget-admin-posts-selector', 'sowPostsSelectorVars', array(
			'modalTitle' => __('Select posts', 'seed'),
		) );
	}
}
...
function siteorigin_widget_post_selector_admin_form_field( $value, $field_name ) {
	...
	<a href="#" class="sow-select-posts button button-secondary">
		...
		<?php esc_html_e( 'Build posts query', 'seed' ) ?>
	</a>
	<?php
}
...
function siteorigin_widget_post_selector_form_fields(){
	...
	// The post type field
	$return['post_type'] = '';
	$return['post_type'] .= '<label><span>' . __('Post type', 'seed') . '</span>';
	$return['post_type'] .= '<select name="post_type">';
	$return['post_type'] .= '<option value="_all">' . __('All', 'seed') . '</option>';
	...
	$return['post__in'] = '';
	$return['post__in'] .= '<label><span>' . __('Post in', 'seed') . '</span>';
	$return['post__in'] .= '<input type="text" name="post__in" class="" />';
	$return['post__in'] .= ' <a href="#" class="sow-select-posts button button-secondary">' . __('Select posts', 'seed') . '</a>';
	$return['post__in'] .= '</label>';

	// The taxonomy field
	$return['tax_query'] = '';
	$return['tax_query'] .= '<label><span>' . __('Taxonomies', 'seed') . '</span>';
	$return['tax_query'] .= '<input type="text" name="tax_query" class="" placeholder="search" />';
	$return['tax_query'] .= '</label>';

	$return['date_query'] = '';
	$return['date_query'] .= '<label><span>' . __('Date range', 'seed') . '</span>';
	$return['date_query'] .= __( 'From', 'seed' );
	$return['date_query'] .= '<input type="date" name="after" class="" />';
	$return['date_query'] .= __( 'To', 'seed' );
	$return['date_query'] .= '<input type="date" name="before" class="" />';
	$return['date_query'] .= '<small>' . __('In the format \'yyyy-mm-dd\'.', 'seed') . '</small>';
	$return['date_query'] .= '</label>';


	// The order by field
	$return['orderby'] = '';
	$return['orderby'] .= '<label><span>' . __('Order by', 'seed') . '</span>';
	$return['orderby'] .= '<select name="orderby">';
	$orderby = array(
		'none' => __('No order', 'seed'),
		'ID' => __('Post ID', 'seed'),
		'author' => __('Author', 'seed'),
		'title' => __('Title', 'seed'),
		'date' => __('Published date', 'seed'),
		'modified' => __('Modified date', 'seed'),
		'parent' => __('By parent', 'seed'),
		'rand' => __('Random order', 'seed'),
		'comment_count' => __('Comment count', 'seed'),
		'menu_order' => __('Menu order', 'seed'),
		'meta_value' => __('By meta value', 'seed'),
		'meta_value_num' => __('By numeric meta value', 'seed'),
		'post__in' => __('By include order', 'seed'),
	);
	...
	$return['posts_per_page'] .= '<label><span>' . __('Posts per page', 'seed') . '</span>';
	$return['posts_per_page'] .= '<input type="number" name="posts_per_page" class="" />';
	$return['posts_per_page'] .= '</label>';


	$return['sticky'] = '';
	$return['sticky'] .= '<label><span>' . __('Sticky posts', 'seed') . '</span>';
	$return['sticky'] .= '<select name="sticky">';
	$sticky = array(
		'' => __('Default', 'seed'),
		'ignore' => __('Ignore sticky', 'seed'),
		'exclude' => __('Exclude sticky', 'seed'),
		'only' => __('Include sticky', 'seed'),
	);
	...
	$return['additional'] = '';
	$return['additional'] .= '<label><span>' . __('Additional', 'seed') . '</span>';
	$return['additional'] .= '<input type="text" name="additional" class="" />';
	$return['additional'] .= '<small>' . __('Additional query arguments. See <a href="http://codex.wordpress.org/Function_Reference/query_posts" target="_blank">query_posts</a>.', 'seed') . '</small>';
...
function siteorigin_widget_post_selector_get_posts_action(){
	...
	foreach($posts->posts as $post) {
		...
		$result['posts'][] = array(
			...
			'id' => $post->ID,
			'thumbnail' => !empty($thumbnail) ? $thumbnail[0] : theme_dir_url(__FILE__).'../css/img/thumbnail-placeholder.png',
