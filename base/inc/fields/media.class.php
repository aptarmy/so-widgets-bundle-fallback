...
class SiteOrigin_Widget_Field_Media extends SiteOrigin_Widget_Field_Base {
	...
	protected function get_default_options() {
		return array(
			'choose' => __( 'Choose Media', 'seed' ),
			'update' => __( 'Set Media', 'seed' ),
			'image_search' => __( 'Image Search', 'seed' ),
			'library' => 'image'
		);
	}
	...
	protected function render_field( $value, $instance ) {
		if( version_compare( get_bloginfo('version'), '3.5', '<' ) ){
			printf( __('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'seed'), admin_url('update-core.php') );
			return;
		}
		...
		<a href="#" class="media-remove-button <?php if( empty( $value ) ) echo 'remove-hide'; ?>"><?php esc_html_e( 'Remove', 'seed' ) ?></a>
	...
	protected function render_after_field( $value, $instance ) {
		if( !empty( $this->fallback ) ) {
			...
			<input type="text" value="<?php echo esc_url( $fallback_url ) ?>"
			       placeholder="<?php esc_attr_e( 'External URL', 'seed' ) ?>"
	...
	function enqueue_scripts(){
		wp_enqueue_script( 'so-media-field', get_template_directory_uri() . '/inc/so-widgets-bundle-fallback/base/inc/fields/' . '/js/media-field' . SOW_BUNDLE_JS_SUFFIX .  '.js', array( 'jquery' ), SOW_FALLBACK_BUNDLE_VERSION );
		wp_enqueue_style( 'so-media-field', get_template_directory_uri() . '/inc/so-widgets-bundle-fallback/base/inc/fields/' . '/css/media-field.css', array( ), SOW_FALLBACK_BUNDLE_VERSION );
	}

	function image_search_dialog(){
		?>
		<script type="text/template" id="so-widgets-bundle-fallback-tpl-image-search-dialog">
			<div class="so-widgets-dialog" id="so-widgets-image-search" data-confirm-import="<?php esc_attr_e( 'Would you like to import this image into your media library?', 'seed' ) ?>">
				...
				<div class="so-widgets-toolbar">
					<h3><?php _e( 'Search For Images', 'seed' ) ?></h3>
				...
				<div class="so-widgets-dialog-frame">
					<div id="so-widgets-image-search-frame">
						...
						<form id="so-widgets-image-search-form">
							<input type="text" value="" name="s" class="widefat so-widgets-search-input" placeholder="<?php echo esc_attr_e( 'Search For Images', 'seed' ) ?>" />
							...
							<div id="so-widgets-image-search-suggestions">
								<strong><?php esc_html_e( 'Related Searches: ', 'seed' ) ?></strong>
						...
						<div id="so-widgets-image-search-powered">
							<?php
							printf(
								__( 'Powered by %s - sponsored results from %s', 'seed' ),
						...
						<div class="so-widgets-results-loading">
							...
							<strong
								data-loading="<?php esc_attr_e( 'Loading Images', 'seed' ) ?>"
								data-importing="<?php esc_attr_e( 'Downloading Image - Please Wait', 'seed' ) ?>"></strong>
						</div>
						<div class="so-widgets-results-more">
							<button class="button-secondary"><?php esc_html_e( 'Load More', 'seed' ) ?></button>
						</div>
		...
		<script type="text/template" id="so-widgets-bundle-fallback-tpl-image-search-result">
			<div class="so-widgets-result">
				<a class="so-widgets-result-image"></a>
			</div>
		</script>

		<script type="text/template" id="so-widgets-bundle-fallback-tpl-image-search-result-sponsored">
