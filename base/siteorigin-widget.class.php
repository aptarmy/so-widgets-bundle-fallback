...
abstract class SiteOrigin_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		<?php if( $this->widget_options['has_preview'] && ! $this->is_customize_preview() ) : ?>
			<div class="siteorigin-widget-preview" style="display: none">
				<a href="#" class="siteorigin-widget-preview-button button-secondary"><?php _e('Preview', 'seed') ?></a>
			</div>
		<?php endif; ?>

		<?php if( !empty( $this->widget_options['help'] ) ) : ?>
			<a href="<?php echo sow_esc_url($this->widget_options['help']) ?>" class="siteorigin-widget-help-link siteorigin-panels-help-link" target="_blank"><?php _e('Help', 'seed') ?></a>
		<?php endif; ?>
	...
	function scripts_loading_message(){
		?>
		<p><strong><?php _e('This widget has scripts and styles that need to be loaded before you can use it. Please save and reload your current page.', 'seed') ?></strong></p>
		<p><strong><?php _e('You will only need to do this once.', 'seed') ?></strong></p>
		<?php
	}
	...
	function enqueue_scripts(){
		...
		if( ! wp_script_is('siteorigin-widget-admin') ) {
			...
			wp_enqueue_style( 'siteorigin-widget-admin', theme_dir_url(SOW_FALLBACK_BUNDLE_BASE_FILE).'base/css/admin.css', array( 'media-views' ), SOW_FALLBACK_BUNDLE_VERSION );

			wp_enqueue_script( 'wp-color-picker' );
			wp_enqueue_media();
			wp_enqueue_script( 'siteorigin-widget-admin', theme_dir_url(SOW_FALLBACK_BUNDLE_BASE_FILE).'base/js/admin' . SOW_BUNDLE_JS_SUFFIX . '.js', array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-slider' ), SOW_FALLBACK_BUNDLE_VERSION, true );
...
=======
		<script type="text/template" id="so-widgets-bundle-fallback-tpl-preview-dialog">
>>>>>>> 63aca95e6d584dc1e331c21b008f7377ec4d235d
			<div class="so-widgets-dialog">
				<div class="so-widgets-dialog-overlay"></div>

				<div class="so-widgets-toolbar">
					<h3><?php _e('Widget Preview', 'seed') ?></h3>
		...
		foreach ( $this->frontend_scripts as $f_script ) {
			if ( ! wp_script_is( $f_script[0] ) ) {
				wp_enqueue_script(
					...
					!empty( $f_script[3] ) ? $f_script[3] : SOW_FALLBACK_BUNDLE_VERSION,
